...
public class BoostPhysioApp {
    ...
    private void initializeSampleData() {
        ...
        physio3.addExpertiseArea("Massage Therapy"); // Ensure no trailing spaces

        Physiotherapist physio4 = new Physiotherapist(4, "Sarah Lee", "111  Walnut St", "555-1212");
        physio4.addExpertiseArea("Sports Therapy");
        physio4.addExpertiseArea("Massage Therapy");

        Physiotherapist physio5 = new Physiotherapist(5, "Tom Harris", "222     Chestnut Ave", "555-2323");
        physio5.addExpertiseArea("Osteopathy");
        physio5.addExpertiseArea("Acupuncture");

        // For the massage treatment
        Treatment massage = new Treatment("Deep Tissue Massage", "Massage Therapy", 60); // Same string
        ...
        // 3. Assign Treatments to Physios (based on expertise)
        ...
        physio4.addTreatment(massage);
        physio4.addTreatment(sports);

        physio5.addTreatment(osteo);
        physio5.addTreatment(acupuncture);
        ...
        for (int day = 0; day < 28; day++) {
            ...
            for (int hour = 9; hour < 12; hour++) {
                ...
                physio4.addTimeSlot(dateStr, new TimeSlot(startTime, endTime));
                physio5.addTimeSlot(dateStr, new TimeSlot(startTime, endTime));
            }
        ...
        // Create patients
        for (int i = 1; i <= 15; i++) {
            Patient p = new Patient(200 + i, "Patient " + i, "Address " + i, "555-0" + i + i + i + i);
            bookingSystem.addPatient(p);
        }

        // 7. Book Sample Appointments
        List<Patient> allPatients = bookingSystem.getAllPatients();
        ...
        List<TimeSlot> slots1 = physio3.getAvailableSlotsForDate(firstDay);
        if (!slots1.isEmpty())
            bookingSystem.bookAppointment(allPatients.get(0), physio3, massage, slots1.get(0));

        List<TimeSlot> slots2 = physio5.getAvailableSlotsForDate(secondDay);
        if (!slots2.isEmpty()) {
            Appointment appt = bookingSystem.bookAppointment(allPatients.get(1), physio5, acupuncture, slots2.get(0));
            appt.markAsAttended();
        }

        List<TimeSlot> slots3 = physio2.getAvailableSlotsForDate(firstDay);
        if (!slots3.isEmpty())
            bookingSystem.bookAppointment(allPatients.get(2), physio2, osteo, slots3.get(0));
    ...
    private void bookByExpertise() {
        ...
        List<Map<String, Object>> availableSlots = bookingSystem.searchAvailableSlotsByExpertise(selectedExpertise);
    ...
    private void bookByPhysiotherapist() {
        ...
        List<Map<String, Object>> availableSlots = bookingSystem
                .searchAvailableSlotsByPhysiotherapist(selectedPhysio.getFullName());
    ...
    private void rescheduleAppointment() {
        ...
        List<Map<String, Object>> availableSlots = bookingSystem
                .searchAvailableSlotsByPhysiotherapist(physio.getFullName());
